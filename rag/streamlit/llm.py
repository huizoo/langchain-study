...
def get_rag_chain():
    ...
    당신은 사용자의 최신 질문을 검색에 적합한 독립 질문으로 바꾸는 역할입니다.

    이전 대화 기록과 최신 질문을 참고해서,
    최신 질문이 이전 대화에 의존하고 있다면 문맥 없이도 이해 가능한 하나의 질문으로 바꾸세요.

    절대 답변하지 마세요.
    질문을 다시 쓰기만 하세요.
    이미 독립적으로 이해 가능한 질문이라면 그대로 반환하세요.
    """
    ...
    system_prompt = """
    당신은 질문-답변 작업을 수행하는 어시스턴트입니다.
    아래에 제공된 검색 문맥을 사용하여 질문에 답변하세요.
